...
describe("renderHudFull (dashboard)", () => {
  ...
  it("should include GEMINI PILOT HUD title", () => {
    const output = stripAnsi(renderHudFull(makeState()));
    expect(output).toContain("GEMINI PILOT HUD");
  });
