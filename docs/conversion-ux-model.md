# Conversion UX Model - Multi-CLI Pilot
...
| Primary buyer or user | Engineering teams and platform groups standardizing multiple coding-agent CLIs. |
| Value wedge | One orchestration harness for Gemini, Qwen, prompts, workflows, hooks, MCP, and team primitives. |
...
| First action | Run the smallest command or fixture that proves npm run verify passes, then inspect the generated output or failure trace. |
...
- Use Workflow completion as the buyer-facing proof metric.
